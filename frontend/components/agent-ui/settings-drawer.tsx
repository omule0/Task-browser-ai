    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          size="icon"
          variant="ghost"
          className="h-10 w-10 sm:h-12 sm:w-12 bg-transparent hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
          aria-label="Open settings"
        >
          <IconSettings size={16} className="sm:w-[18px] sm:h-[18px]" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[280px] sm:w-80 md:w-96 p-0 bg-background border-border" align="end" sideOffset={8}>
